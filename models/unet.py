...
class UNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=3, base_c=None):
        super(UNet, self).__init__()

        base_c = cfg.filter_val if base_c is None else base_c
        ...
        self.in_conv1 = FirstFeature(n_channels, c1)
        self.in_conv2 = ConvBlock(c1, c1)

        self.enc_1 = Encoder(c1, c2)
        self.enc_2 = Encoder(c2, c3)
        self.enc_3 = Encoder(c3, c4)
        self.enc_4 = Encoder(c4, c5)

        # Важно: первый аргумент Decoder = каналы ПОСЛЕ конкатенации (т.е. in+skip)
        self.dec_1 = Decoder(c5, c4)   # после 1x1 станет c4, concat с skip c4 -> c5, ConvBlock(c5 -> c4)
        self.dec_2 = Decoder(c4, c3)
        self.dec_3 = Decoder(c3, c2)
        self.dec_4 = Decoder(c2, c1)

        self.out_conv = FinalOutput(c1, n_classes)
